...
void DropDown::RefreshPopup (HMENU menu, int barItemNo)
{
	if (barItemNo < 0 || barItemNo >= _barItemCnt)
		throw Win::Exception ("Internal error: Unknown menu initialization");
...
Context::Context (std::vector<Menu::Item const *> const & templ, Cmd::Vector & cmdVector)
{
		 ...
		 ++iter)
	{
		...
		int curItem = 0;
		if (curTempl->kind != END)
		{
			if (curTempl->kind == CMD)
			{
				// Add only enabled items
				Cmd::Status cmdStatus = cmdVector.Test (curTempl->cmdName);
				if (cmdStatus != Cmd::Disabled)
				{
					...
					AddItem (cmdVector.CmdToId (curTempl->cmdName), 
							curTempl->displayName);
				}
			}
			else
			{
				assert (curTempl->kind == SEPARATOR);
				if (!firstSeparator && !separatorPending)
				{

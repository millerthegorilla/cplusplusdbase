...
namespace Pen
{
	...
	public:
		...
		operator HPEN () const { return reinterpret_cast<HPEN> (_obj); }
	};
...
namespace Font
{
	...
	class SysFont: public Stock
	{
	public:
		SysFont () : Stock (SYSTEM_FONT) {}
	};
	...
	public:
		...
		public:
			Color (HDC hdc, int r, int g, int b)
				: _hdc (hdc), _oldColor (::SetTextColor (_hdc, RGB (r, g, b)))
			{}
			~Color ()
			{
		...
		public:
			Background (HDC hdc, int r, int g, int b)
				: _hdc (hdc), _oldColor (::SetBkColor (_hdc, RGB (r, g, b)))
			{}
			~Background ()
			{
		...
		{
		public:
			SysFixed (HDC hdc): Win::StockObjectHolder (hdc, SYSTEM_FIXED_FONT) {}
		};
	};
}
